...
In this work, authors propose to learn language modeling from multi-token prediction rather than next-token prediction. At each position of the training corpus, the model is instructed to predict $n$ future tokens at once. Thus, training objective can be expressed as follow:
